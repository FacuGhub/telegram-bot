...
def parsear_mensaje(texto_raw: str) -> dict:
    ...
    if not re.fullmatch(r"\d+", cantidad_txt.strip()):
        raise ValueError("CANTIDAD_FORMATO")

    cantidad_num = int(cantidad_txt.strip())
    if cantidad_num < 0:
        raise ValueError("CANTIDAD_NEGATIVA")

    # Vendedores:
    # - si viene vacío o "-", se normaliza a "-"
    # - si vienen nombres, se aceptan separados por coma o punto y coma
    if vendedores_txt.strip() in ("", "-"):
        vendedores = "-"
    else:
        vendedores = ", ".join(
    ...
    return {
        ...
        "cantidad": str(cantidad_num),
        "vendedores": vendedores,
        "comentarios": comentarios.strip(),
    }

def enviar_a_forms(datos: dict) -> dict:
    if not FORM_URL:
        raise RuntimeError("FORM_URL no configurado")
    ...
    logging.info(
        "Enviando a Google Forms | fecha=%s | capacitador=%s | cadena=%s | zona=%s | direccion=%s",
        datos["fecha"],
        datos["capacitador"],
        datos["cadena"],
        datos["zona"],
    ...
    logging.info(
        "Google Forms OK | status=%s | final_url=%s",
        info["status_code"],
        info["final_url"],
    )
    return info
...
async def procesar_mensaje(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if test_mode:
            ...
            try:
                forms_result = enviar_a_forms(datos)
                sent_to_forms = 1
        ...
        errores = {
            "FORMATO": "Debes enviar 7 u 8 líneas (Comentarios opcional).",
            ...
            "CANTIDAD_FORMATO": "Cantidad debe ser un número entero (0 o más).",
            "CANTIDAD_NEGATIVA": "Cantidad debe ser 0 o más.",
                    }
